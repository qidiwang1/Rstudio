...
#为ICL补数据，NA都补为0
merge <- data.frame()
for (a in levels(factor(data$iso3_name))){
  country <- data[data$iso3_name==a,]
  aaa <- country[!is.na(country$ICL),]
  if (sum(nrow(aaa)==0)){
    country <- country
  } else{
    country$ICL[is.na(country$ICL)] <- 0
    country$ICL_total[is.na(country$ICL_total)] <- 0
  }
...
analysis_data <- data[,c(3,25,5:7,12:14,19,62,63,65:66,76:78)]
...
write_xlsx(cluster_result,'/Users/qidiwang1/Desktop/time_series_cluster_reslut_nolimit.xlsx')
