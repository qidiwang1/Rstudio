...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 31 20:30:57 2020
...
writer.writerow(('job','company','salary','area','experience',\
                 'education','number','time','companytype','direction'))
...
def get_content(link):
    ...
    #print(salary)
    if len(t1.xpath('//p[@class="msg ltype"]/text()')) <= 4: 
        area = t1.xpath('//p[@class="msg ltype"]/text()')[0].strip()
        experience = ''
        education= t1.xpath ('//p[@class="msg ltype"]/text()')[1].strip()
        number= t1.xpath ('//p[@class="msg ltype"]/text()')[2].strip()
        time = t1.xpath ('//p[@class="msg ltype"]/text()')[3].strip()
    elif len(t1.xpath('//p[@class="msg ltype"]/text()')) >= 5: 
        area = t1.xpath('//p[@class="msg ltype"]/text()')[0].strip()
        experience = t1.xpath('//p[@class="msg ltype"]/text()')[1].strip()
        education= t1.xpath ('//p[@class="msg ltype"]/text()')[2].strip()
        number= t1.xpath ('//p[@class="msg ltype"]/text()')[3].strip()
        time = t1.xpath ('//p[@class="msg ltype"]/text()')[4].strip()
    ...
    writer.writerow((job,company,salary,area,experience,education,number,time,companytype,direction))
    return True
 
#主调动函数
#爬取前三页信息
for i in range(208,2000): 
