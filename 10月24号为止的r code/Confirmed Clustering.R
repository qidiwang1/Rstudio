a <- read.csv('/Users/qidiwang1/Desktop/疫情数据库/processed_data.csv')
...
for (i in levels(a[,2])){
  ...
  if (nrow(b3)>=21)
    bbbb <- rbind(bbbb,b3[c((nrow(b3)-20):nrow(b3)),]) #1:21
}
View(bbbb)
...
col_name <- data.frame(as.character(c(1:21)))
for (i in levels(bbbb[,3])){
  b3 <- data.frame()
  for (j in c(1:nrow(bbbb))) {
    if (bbbb[j,3]==i)
...
View(cluster_data)
cluster_data.pca <- prcomp(log(cluster_data+1), center =TRUE, scale = TRUE)
summary(cluster_data.pca)

library(FactoMineR)
res.pca <- PCA(log(cluster_data+1), ncp = 2, graph =TRUE)
